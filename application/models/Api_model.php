...
class Api_model extends CI_Model {
   ...
   public function user_menu($type){
										...
										foreach($subadmin_menu as $subarr => $list_mainmenu2){
													...
													//if($sub_url == strtolower($this->uri->segment(1))){
														...
														if(sizeof($this->uri->segments) > 1){
															...
															/*if($sub_mainparent == 95){
																	$classactive = 'class="active '.$segment.' '.$sub_mainparent.' '.$admin_menu_id2.'"';
															}else*/
    ...
    public function get_picture($ref_id, $picid = 0, $ref_type = 1 ){
	    	...
	    	$this->db->where('ref_id', intval($ref_id));
	    	$this->db->where('ref_type', intval($ref_type));
	    	//$this->db->where('status', $pic_status);
	    	
	    	if($picid != 0) $this->db->where('picture_id', intval($picid));
    ...
    public function get_item($type = 0, $ref_id, $picid = 0){
		...
		$this->db->select('g.*, a.title, a.create_date');
	    $this->db->from('_gallery_set g');
	
		$this->db->join('_article a', 'a.article_id2 = g.ref_id and a.lang = "'.$language.'"', 'left');
		//$this->db->join('_news_old no', 'no.news_id2 = p.ref_id', 'left');
		...
		$this->db->order_by('news_id', 'ASC');*/
	    	...
	    	$this->db->where('ref_id', intval($ref_id));
	    	if($type > 0) $this->db->where('ref_type', $type);
	    	
	    	//$this->db->where('status', $pic_status);
	    	
	    	if($picid != 0) $this->db->where('galleryset_id', $picid);

		//if($lang != '') $this->db->where('_news.lang', $language);

	    	$this->db->order_by('g.order', 'ASC');
    ...
    function get_highlight($ref_type = 0, $id = 0,  $debug = 0){
			...
			$this->db->select('_highlight.order, _highlight.highlight_id, 	_highlight.article_id, _highlight.ref_type, _gallery_set.file_name, _gallery_set.folder,  _gallery_set.url,
			dd.create_date as folder_news ,dd.create_date, dd.lastupdate_date, dd.status, dd.approve');
			...
			switch(intval($ref_type)){
					...
					default: 
						if($id > 0)
							$this->db->join('_article dd', '_highlight.article_id = dd.article_id2 and '.$prefix.'_highlight.ref_type = 1 and dd.article_id2 = '.$id); 
						else
							$this->db->join('_article dd', '_highlight.article_id = dd.article_id2 and '.$prefix.'_highlight.ref_type = 1'); 
			...
			$this->db->join('_gallery_set', '_highlight.article_id = _gallery_set.ref_id and '.$prefix.'_gallery_set.ref_type = '.$prefix.'_highlight.ref_type AND '.$prefix.'_gallery_set.`default`=1 ', 'left');
    ...
    function get_editorpick($ref_type = 0, $id = 0,  $debug = 0){
			...
			$this->db->select('_editor_picks.order, _editor_picks.editor_picks_id, 	_editor_picks.article_id, _editor_picks.ref_type, _gallery_set.file_name, _gallery_set.folder,  _gallery_set.url,
			dd.create_date as folder_news ,dd.create_date, dd.lastupdate_date, dd.status, dd.approve');
			$this->db->select('dd.title, dd.countview, dd.category_id, _category.category_name, _category.type');
			$this->db->select('dd.subcategory_id, sub.subcategory_name');
			...
			switch(intval($ref_type)){
					...
					default: 
						if($id > 0)
							$this->db->join('_article dd', '_editor_picks.article_id = dd.article_id2 and '.$prefix.'_editor_picks.ref_type = 1 and dd.article_id2 = '.$id); 
						else
							$this->db->join('_article dd', '_editor_picks.article_id = dd.article_id2 and '.$prefix.'_editor_picks.ref_type = 1'); 
			...
			$this->db->join('_gallery_set', '_editor_picks.article_id = _gallery_set.ref_id and '.$prefix.'_gallery_set.ref_type = '.$prefix.'_editor_picks.ref_type AND '.$prefix.'_gallery_set.`default`=1 ', 'left');
			...
			$this->db->order_by('_editor_picks.order', 'Asc');
			$this->db->limit($listpage, $limit_start);
