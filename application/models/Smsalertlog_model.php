 ...
 * @package codeigniter.application.models
 */
class Smsalertlog_Model extends CI_Model{
    ...
    public function get_max_id(){
		...
		$this->db->from('_sendmsms_alert_log');
		$query = $this->db->get();
	...
	public function getsmsalertlog($pageIndex = 1, $limit = 100,$startdate= null,$enddate= null) {
		$language = $this->lang->language['lang'];
		$this->db->cache_on();
		//$this->db->count_all('_sendmsms_alert_log');
		//$this->db->distinct();
		$this->db->select('*');
		$this->db->from('_sendmsms_alert_log');
		//$this->db->join('_hardware', 'sd_hardware.hardware_name = sd_sendmsms_alert_log.sensor_hwname');
	...
	public function viewMenu($id = 0, $parent = '', $status = ''){
		...
		$this->db->select('*');
		$this->db->from('_sendmsms_alert_log');
		if($id > 0) $this->db->where('sms_id', $id);
    ...
    function store($sms_id = 0, $data){

			//echo "(sms_id = $sms_id)";

			if($sms_id > 0){
					$this->db->where('sms_id', $sms_id);
					$this->db->update('_sendmsms_alert_log', $data);
					$report = array();
					...
					$this->db->insert('_sendmsms_alert_log', $data);
					//echo $this->db->last_query();
	...
	function status_new($sms_id, $enable = 1){
	...
	function update_order($sms_id, $order = 1, $parent = 0){

		$data['order_by'] = $order;
		$this->db->where('sms_id', intval($sms_id));
		$this->db->update('_sendmsms_alert_log', $data);
		//Debug($this->db->last_query());
	...
	function update_orderid_to_down($order, $max, $parent = 0){
		...
		$this->db->update('_sendmsms_alert_log');
		//Debug($this->db->last_query());
