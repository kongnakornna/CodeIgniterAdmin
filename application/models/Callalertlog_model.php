 ...
 * @package codeigniter.application.models
 */
class Callalertlog_model extends CI_Model{
    ...
    public function get_max_id(){
		...
		$this->db->select('max(call_id) as max_id');
		$this->db->from('_call_alert_log');
		$query = $this->db->get();
		return $query->result_array(); 

    }

 	public function totalsmsalart($startdate=NULL,$enddate= NULL){
		$this->db->distinct();
		$this->db->select('*');
		$this->db->from('_call_alert_log');
	...
	public function getcallalertlog($pageIndex = 1, $limit = 100,$startdate= null,$enddate= null) {
		$language = $this->lang->language['lang'];
		$this->db->cache_on();
		//$this->db->count_all('_call_alert_log');
		//$this->db->distinct();
		$this->db->select('*');
		$this->db->from('_call_alert_log');
		//$this->db->join('_hardware', 'sd_hardware.hardware_name = sd_call_alert_log.sensor_hwname');
	...
	public function viewMenu($id = 0, $parent = '', $status = ''){
		...
		$this->db->select('*');
		$this->db->from('_call_alert_log');
		if($id > 0) $this->db->where('call_id', $id);
    ...
    function store($call_id = 0, $data){

			//echo "(call_id = $call_id)";

			if($call_id > 0){
					$this->db->where('call_id', $call_id);
					$this->db->update('_call_alert_log', $data);
					$report = array();
					...
					$this->db->insert('_call_alert_log', $data);
					//echo $this->db->last_query();
	...
	function status_new($call_id, $enable = 1){
	...
	function update_order($call_id, $order = 1, $parent = 0){

		$data['order_by'] = $order;
		$this->db->where('call_id', intval($call_id));
		$this->db->update('_call_alert_log', $data);
		//Debug($this->db->last_query());
	...
	function update_orderid_to_down($order, $max, $parent = 0){
		...
		$this->db->update('_call_alert_log');
		//Debug($this->db->last_query());
