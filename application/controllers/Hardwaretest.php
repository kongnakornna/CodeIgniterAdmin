...
class Hardwaretest extends MY_Controller {
	...
	public function add($id=1){
			...
			$language = $this->lang->language;			
			$breadcrumb[] = '<a href="'.base_url('hardwaretest/listview').'">'.$language['hardwaretest'].'</a>';
	...
	public function listview($pageIndex=1){
		...
		$language = $this->lang->language;
          ...
          $breadcrumb[] = '<a href="'.base_url('electricitytype').'">'.$language['electricitytype'].'</a>';		$breadcrumb[] = '<a href="'.base_url('waterpipe').'">'.$language['waterpipe'].'</a>';
          
		$breadcrumb[] = '<a href="'.base_url('hardwaretest/add').'">'.$language['add'].$language['hardwaretest'].'</a>';
	...
	public function run($id=1){
		    if($this->session->userdata('admin_type')>'2'){ redirect(base_url()); }
		     $ListSelect = $this->Api_model_na->user_menu($this->session->userdata('admin_type'));		
			$language = $this->lang->language;			
			$breadcrumb[] = '<a href="'.base_url('hardwaretest/run').'">'.$language['hardwaretest'].'</a>';
			//"admin_menu" => $this->menufactory->getMenu(),
			//$breadcrumb[] = $language['add'];
			$this->load->model('Hardwaretestrun_model');
	...
	public function hardwaretest_run($id = 0){
			$ListSelect = $this->Api_model_na->user_menu($this->session->userdata('admin_type'));		
			$language = $this->lang->language;	
			$breadcrumb[] = '<a href="'.base_url('hardwaretest').'">'.$language['hardwaretest'].'</a>';
