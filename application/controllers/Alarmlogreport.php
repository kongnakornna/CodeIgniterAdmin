...
*/
class Alarmlogreport extends CI_Controller {

    public function __construct()    {
        parent::__construct();
        // load helper
		$this->load->helper('url');
        $this->load->model('Alarmlogreport_Model');
	...
	$enddate = $this->input->get_post('enddate',TRUE);
	   ...
	   // $pagination = $this->session->userdata($segment);
		...
		// Pagination  
		$total_rows= $this->Alarmlogreport_Model->totalsensorlog($startdate,$enddate);
		...
		//$pageConfig = doPagination($this->Alarmlogreport_Model->totalsensorlog($startdate,$enddate), $limit, $segment,$startdate,$enddate, site_url("/alarmlogreport/listview").$search_key);
		$pageConfig = doPagination($this->Alarmlogreport_Model->totalsensorlog($startdate,$enddate), $limit, $segment,$startdate,$enddate, site_url("/alarmlogreport/listview"));
		//$pageConfig=$pageConfig;//.$search_key;
		}else{
		$pageConfig = doPagination($this->Alarmlogreport_Model->totalsensorlog($startdate,$enddate), $limit, $segment, site_url("/alarmlogreport/listview/"));
		}
		//Debug($pageConfig);
		//die();
		...
		if($startdate!==''){
		$web_menu = $this->Alarmlogreport_Model->getalarmlogreport($pageIndex, $limit,$startdate,$enddate);
		}else{
		$web_menu = $this->Alarmlogreport_Model->getalarmlogreport($pageIndex, $limit);
		}
		
		//$web_menu = $this->Alarmlogreport_Model->getalarmlogreport($limit,$start * $limit);
		  //Debug($web_menu);
		  //die();
