...
*/
class Sensormanage extends MY_Controller {

    public function __construct()    {
        parent::__construct();
          $this->load->model('Sensormanage_model');
	...
	public function listview($pageIndex=1){
		...
		$breadcrumb[] = '<a href="'.base_url('sensormanage/add').'">'.$language['add'].$language['sensormanage'].'</a>';
		$breadcrumb[] = $language['sensormanage'];
          $total_rows= $this->Sensormanage_model->total($startdate,$enddate);
		...
		$pageConfig = doPagination($this->Sensormanage_model->total($startdate,$enddate), $limit, $segment,$startdate,$enddate, site_url("/sensormanage/listview"));
		}else{
		...
		if($startdate!==''){
		$sensormanage_list = $this->Sensormanage_model->get_condition_group($pageIndex, $limit,$startdate,$enddate);
		}else{
		$sensormanage_list = $this->Sensormanage_model->get_condition_group($pageIndex, $limit);
		}
		  //Debug($sensormanage_list);
		  //die();
		 $links=$this->pagination->create_links();
		 //$links=$this->pagination->create_links($limit, $start);
		 //Debug($links);
		 //die();
			//////////$sensormanage_list = $this->Sensormanage_model->get_condition_group();
			//Debug($sensormanage_list);
			//die();

			$data = array(				
                    "sensormanage_list" => $sensormanage_list,
				"content_view" => 'tmon/sensormanage',
	...
	public function add(){
			...
			$ListSelect = $this->Api_model_na->user_menu($this->session->userdata('admin_type'));		
			$language = $this->lang->language;			
			$breadcrumb[] = '<a href="'.base_url('sensormanage').'">'.$language['sensormanage'].'</a>';
			//"admin_menu" => $this->menufactory->getMenu(),
			$breadcrumb[] = $language['add'];
			$data = array(						
				"content_view" => 'tmon/sensormanage_add',
				"ListSelect" => $ListSelect,
				"breadcrumb" => $breadcrumb
			);
			$this->load->view('template/template',$data);
	...
	public function edit($id = 0){
			...
			$sensormanage_list = $this->Sensormanage_model->get_condition_groupe_edit($id);
               //Debug($sensormanage_list);Die();
			$data = array(						
				"sensormanage" => $sensormanage_list,
				"content_view" => 'tmon/sensormanage_edit',
				"ListSelect" => $ListSelect,
				"breadcrumb" => $breadcrumb
			);
               //Debug($data); die();
	...
	public function status($id){
		...
		//$id = $this->input->post("id");
			...
			//echo $id;die();
			$obj_status = $this->Sensormanage_model->get_status2($id);
			$cur_status = $obj_status[0]['status'];
			$sensormanage_name = $obj_status[0]['condition_group_name'];
			//Debug($cur_status);
			
			if($cur_status == 0) $cur_status = 1;
			else $cur_status = 0;
			$this->Sensormanage_model->status_condition_group($id, $cur_status);
          ...
          //**************Log activity
			...
			$savedata = $language['savedata'];
               ...
               $ref_title=$sensormanage_name." [Status ".$cur_status."]";
               $action=2;
	...
	public function update(){
			...
			$breadcrumb[] = $language['sensormanage'];
			 ...
			 if ($this->input->server('REQUEST_METHOD') === 'POST'){
					...
					$this->Sensormanage_model->store($data_access['condition_group_id'], $data_to_store);
                    ...
                    //**************Log activity
          			...
          			$savedata = $language['savedata'];
                         ...
                         ########IP#################
                         $ref_type=1;
                         $ref_title="Admin sensormanage : ".$data_access['condition_group_name']." Grant Access";;
                         $action=2;
                         $create_date=date('Y-m-d H:i:s');
                         $status=1;
                    	$log_activity = array(
                    ...
                    redirect('sensormanage');
			 }

			$sensormanage_list = $this->Sensormanage_model->get_condition_group();
			$data = array(				
					"sensormanage" => $sensormanage_list,
					"content_view" => 'tmon/sensormanage_update',
					"ListSelect" => $ListSelect,
					"breadcrumb" => $breadcrumb,
	...
	public function save(){
        ...
        if ($this->input->server('REQUEST_METHOD') === 'POST'){
				...
				$forbiddenna=$language['forbidden'];
					...
					echo '<script language="javascript">alert("'.$housenn1.'");top.location="'.base_url('sensormanage/edit/'.$condition_group_id2).'"; window.history.back(-1);</script>';	
					}else if($condition_group_id2=='' || $condition_group_id2==0){
					echo '<script language="javascript">alert("'.$housenn1.'");top.location="'.base_url('sensormanage/add/').'"; window.history.back(-1);</script>';	
					}
					exit();
...
echo '<script language="javascript">alert("'.$minutenn1.'");top.location="'.base_url('sensormanage/edit/'.$condition_group_id2).'";</script>';	
				 }else if($condition_group_id2=='' || $condition_group_id2==0){
echo '<script language="javascript">alert("'.$minutenn1.'");top.location="'.base_url('sensormanage/add/').'";</script>';	
				 }
				 exit();
				}}#################****Date****###############
...
if($condition_group_id2!=='' || $condition_group_id2!==0){
 ;
echo '<script language="javascript">alert("'.$datenn1.'");top.location="'.base_url('sensormanage/edit/'.$condition_group_id2).'";</script>';
				 }else if($condition_group_id2=='' || $condition_group_id2==0){
echo '<script language="javascript">alert("'.$datenn1.'");top.location="'.base_url('sensormanage/add/').'";</script>';		
				 }
				 exit();
				}#################****Month****###############
...
if($condition_group_id2!=='' || $condition_group_id2!==0){
 ;
echo '<script language="javascript">alert("'.$monthnn1.'");top.location="'.base_url('sensormanage/edit/'.$condition_group_id2).'";</script>';	
				 }else if($condition_group_id2=='' || $condition_group_id2==0){
echo '<script language="javascript">alert("'.$monthnn1.'");top.location="'.base_url('sensormanage/add/').'";</script>';		
				 }
				 exit();
				}################################
				...
				$order_by = $this->Sensormanage_model->get_max_order();
				$get_max_id = $this->Sensormanage_model->get_max_id();
				...
				if ($this->form_validation->run()){
						...
						if($condition_group_id2 > 0){ //UPDATE SQL
								...
								if($this->Sensormanage_model->store($condition_group_id2,$lang_en, $data_to_store_en)){
									$data['flash_message'] = TRUE; 
                                                  //**************Log activity
                                        			...
                                        			$savedata = $language['savedata'];
                                                       ...
                                                       ########IP#################
                                                       $ref_type=1;
                                                       $ref_title=$edit." ID ".$condition_group_id2." sensormanage : ".$condition_group_name_en;
                                                       $action=2;
                                                       $create_date=date('Y-m-d H:i:s');
                                                       $status=1;
                                                  	$log_activity = array(
								...
								//////////////////////
                                        //if the insert has returned true then we show the flash message
								if($this->Sensormanage_model->store($condition_group_id2,$lang_th, $data_to_store_th)){
									$data['flash_message'] = TRUE; 
                                             //**************Log activity
                                             	$language = $this->lang->language;
                                   			$edit = $language['edit'];
                                   			$savedata = $language['savedata'];
                                                  ...
                                                  ########IP#################
                                                  $ref_type=1;
                                                  $ref_title=$edit." ID ".$condition_group_id2." sensormanage : ".$condition_group_name_th;
                                                  $action=2;
                                                  $create_date=date('Y-m-d H:i:s');
                                                  $status=1;
                                             	$log_activity = array(
							...
							//if the insert has returned true then we show the flash message
								if($this->Sensormanage_model->store_product($data_to_store)){
									$data['flash_message'] = TRUE;
                                        //**************Log activity
                                        	$language = $this->lang->language;
                              			$add = $language['add'];
								...
								if($this->Sensormanage_model->store_product($data_to_store_th)){
									$data['flash_message'] = TRUE;
                                             //**************Log activity
                                        	$language = $this->lang->language;
                              			$add = $language['add'];
						...
						$sensormanage_name = "Update [Fail]";
						//$data['error'] = '<div class="alert alert-error"><a class="close" data-dismiss="alert">×</a><strong>\', \'</strong></div>';
						//$data['main_content'] = 'basic/sensormanage/add';
						//"admin_menu" => $this->menufactory->getMenu(),
						$data = array(									
									"content_view" => 'sensormanage/add',
									"ListSelect" => $ListSelect,
        ...
        //load the view
        //$data['main_content'] = 'basic/sensormanage';
        //$this->load->view('template/template',$data);
		if ($this->form_validation->run()) 
			 redirect('sensormanage');
	...
	public function delete($id){
			...
			$OBJnews = $this->Sensormanage_model->get_status($id);
			//Debug($OBJnews);
			//die();
			$condition_group_name = $OBJnews[0]['condition_group_name'];
			//$order_by = $OBJnews[0]['order_by'];
               if($id<=6){
                  $this->Sensormanage_model->delete_sensormanage($id); //Update 
               }elseif($id>6){
                   $this->Sensormanage_model->delete_sensormanage_by_admin($id); //Delete database
               }
			
			//**************Order New
			//$this->Sensormanage_model->update_orderdel($order_by);
          ...
          //**************Log activity
			...
			$savedata = $language['savedata'];
               ...
               ########IP#################
               $ref_type=1;
               $ref_title="Delete ID ".$id." sensormanage : ".$condition_group_name;
               $action=2;
               $create_date=date('Y-m-d H:i:s');
               $status=1;
          	$log_activity = array(
