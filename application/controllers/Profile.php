...
function welcome(){	
	...
	function memberlist(){
			...
			$admin_type_id = ($this->input->post('admin_type_id')) ? $this->input->post('admin_type_id') : 0;

			$search_form = $this->input->post();
			//Debug($search_form);
			if(isset($search_form['keyword'])){
				$keyword = trim($search_form['keyword']);
				$memberlist = $this->adminfactory->getAdmin(0, $keyword);
			}else{
				$memberlist = $this->adminfactory->getAdmin(0, null, $admin_type_id);
			}

			//Debug($memberlist);
			$getAdminType = $this->adminfactory->getAdminType();
			//Debug($getAdminType);
			$admin_type_list = $this->Admin_model->getSelectAdminType($admin_type_id, $getAdminType);
	...
	public function status(){
			...
			echo $cur_status;
				...
				$log_activity = array(
									...
									"ref_id" => $id,
									"ref_type" => 0,
									"ref_title" => $edit." Admin Username : ".$member->_admin_username." [Status] Update",
									"action" => $action
				);			
				$this->Admin_log_activity_model->store($log_activity);
				//**************Log activity
