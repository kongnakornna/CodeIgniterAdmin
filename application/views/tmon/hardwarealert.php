...
<hr />
 ...
 <center> 
<div class="panel-body"style="padding: 1px 5px;"> <?php  echo $pagination;?> </div>  
</center>


<?php
	$attributes = array('class' => 'form-horizontal', 'id' => 'jform');
	echo form_open('hardwarealert', $attributes);
?>
...
<div class="row"> <div class="col-md-12 space20"> <div class="btn-group pull-right">

<?php //echo $this->lang->line('search');?>&nbsp;

<span class="input-icon">
<input name="startdate" type="text" class="some_class form-control" id="some_class_1" value="<?php echo $startdate;?>" placeholder="<?php echo $this->lang->line('datestart');?>"/>
...
<input name="enddate" type="text" class="some_class form-control" id="some_class_2" value="<?php echo $enddate; ?>"placeholder="<?php echo $this->lang->line('enddate');?>"/>
<i class="fa fa-hand-o-left"></i> </span>
&nbsp;&nbsp;
	<input type="submit" value="<?php echo $this->lang->line('search');?>" class="btn btn-green pull-right">
...
<div>
	...
	<thead>
		<tr>
														<th width="2%"><i class="ace-icon fa fa-cog bigger-110 hidden-480"></i></th>
														<th width="3%"><i class="ace-icon fa fa-sort-numeric-desc bigger-110 hidden-480"></i>
														<?php //echo $language['no'] ?></th>
														<th width="24%"><i class="ace-icon fa fa-sort-amount-desc bigger-110 hidden-480"></i>
														<?php echo $language['title'] ?></th>
														<th width="27%"><i class="ace-icon fa fa-sitemap bigger-110 hidden-480"></i>
														<?php echo $language['hardware'] ?>  </th>
														<th width="12%"><?php echo $language['alerttime'] ?></th>
														<!--
														<th width="15%">
														<i class="ace-icon fa fa-clock-o bigger-110 hidden-480"></i>
														<?php #echo $language['create_date'] ?></th>
														-->
														<th width="12%"><i class="ace-icon fa fa-tint bigger-110 hidden-480"></i><?php echo $language['action'] ?></th>
					</tr>
				  </thead>
				<tbody>
<?php
				...
				for($key=0;$key<$maxcat;$key++){
					...
					$hardware_name=$hardware_list[$key]['hardware_name'];
					$hardware_decription=$hardware_list[$key]['hardware_decription'];
					$hardgroup_name=$hardware_list[$key]['hardgroup_name'];
					$hardware_type_name=$hardware_list[$key]['hardware_type_name'];
...
?>
		<tr>
						...
						<?php echo $hardware_id_map;?><input type="hidden" class="ace" name="selectid[]" value="<?php echo $hardware_id_map;?>"/>
						</span>						</td>
						<td><?=$i?> </td>
						<td><i class="fa fa-desktop "></i>
						<?php 
						 if($status== 1){
						 echo "<font color='blue'> $hardware_name IP: $hardware_ip : $port </font>";
						 }else{ 
						 echo "<font color='red'> $hardware_name  IP: $hardware_ip : $port </font>"; 
						 }
						 ?>						</td>
						<td>
...
<?php 
						 if($status== 1){
						 echo "<font color='Green'> $hardware_type_name</font> ($hardware_decription)";
						 }else{ 
						 echo "<font color='red'> $hardware_type_name </font> ($hardware_decription)"; 
						 }
						 ?>						</td>
						<td><?php echo "<font color='red'> $alerttime </font>";  ?> </td>
						
<!--
<td><?php /*
if($lang=='th'){$create_date=RenDateTime($create_date); 
 if($status== 1){echo "<font color='blue'> $create_date </font>";}else{ echo "<font color='red'> $create_date </font>"; }
}else{
 if($status== 1){echo "<font color='blue'> $create_date </font>";}else{ echo "<font color='red'> $create_date </font>"; }
} 
*/ ?></td>
						...
						<?php #########################?>		
												<div>
													<div class="btn-group">
														...
														<?php if($hardware_list[$key]['status'] == 1 || $hardware_list[$key]['status'] == 0){?>
															<li role="presentation">
																<a role="menuitem" tabindex="-1" href="<?php echo site_url('hardwarealert/edit/'.$hardware_list[$key]['hardware_id_map']); ?>">
...
<?php
		if($hardware_list)		
				for($key=0;$key<$maxcat;$key++){		
						$title = $hardware_name;
?>
		$('#status<?=$hardware_list[$key]['hardware_id_map']?>').on('click', function() {
				var id = $(this).attr('id');
		    	var n = id.length;
			    var maxstr = n-6;
				...
				$.ajax({
					type: 'POST',
					url:  '<?php echo base_url('hardwarealert/status/'.$hardware_list[$key]['hardware_id_map'])?>',
					cache: false,
